...
class APIManager: SessionManager {
    ...
    static let consumerKey = "uFTmFW66AAMEUwx3rZlZDMSCf"  // YOUR_KEY_HERE
    static let consumerSecret = "LtlxIoQpBvHcqjpSMIA9Gs2E9wCJbr7xkx9EpSdBYoNedaZUgh" // YOUR_SECRET_HERE
...
//
        ...
        oauthManager.authorize(withCallbackURL: callbackURL, success: { (credential, _response, parameters) in
            ...
            self.getCurrentAccount(completion: { (user, error) in
                if let error = error {
                    ...
                    print("Welcome \(user.name ?? "Unknown User")")
                    User.current = user
                    success()
                }
            })
    ...
    func getHomeTimeLine(max_id: Int?,completion: @escaping ([Tweet]?, Error?) -> ()) {
        let param = [
            "tweet_mode" : "extended",
            "max_id" : max_id!
            ] as [String : Any]
        
        request(URL(string: "https://api.twitter.com/1.1/statuses/home_timeline/.json")!, method: .get, parameters: param)
            .validate()
            .responseJSON { (response) in
                switch response.result {
                case .failure(let error):
                ...
                case .success:
                    ...
                    print(tweetDictionaries.capacity)
                    print(tweetDictionaries.count)
...
//        }
    ...
    func getUserTimeLine(completion: @escaping ([Tweet]?, Error?) -> ()) {
        request(URL(string: "https://api.twitter.com/1.1/statuses/user_timeline.json")!, method: .get, parameters: param)
            .validate()
            .responseJSON { (response) in
                switch response.result {
                case .failure(let error):
                ...
                let data = NSKeyedArchiver.archivedData(withRootObject: tweetDictionaries)
                UserDefaults.standard.set(data, forKey: "user_timeline_tweets")
                UserDefaults.standard.synchronize()
                let tweets = tweetDictionaries.flatMap({ (dictionary) -> Tweet in
                    Tweet(dictionary: dictionary)
                })
    ...
    func getUserFavorites(completion: @escaping ([Tweet]?, Error?) -> ()) {
        request(URL(string: "https://api.twitter.com/1.1/favorites/list.json")!, method: .get, parameters: param)
            .validate()
            .responseJSON { (response) in
                switch response.result {
                case .failure(let error):
                ...
                case .success:
                    ...
                    let data = NSKeyedArchiver.archivedData(withRootObject: tweetDictionaries)
                    UserDefaults.standard.set(data, forKey: "favorites_tweets")
                    UserDefaults.standard.synchronize()
                    let tweets = tweetDictionaries.flatMap({ (dictionary) -> Tweet in
                        Tweet(dictionary: dictionary)
                    })
